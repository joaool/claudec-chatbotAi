...
export default function DocumentManager() {
  ...
  useEffect(() => {
    fetch('/api/admin/assistants')
      .then((r) => r.json())
      .then((data) => {
        const list: AssistantOption[] = data.assistants ?? [];
        setAssistants(list);
  ...
  const fetchFiles = useCallback((assistantId: string) => {
    return fetch(`/api/admin/documents?assistantId=${assistantId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  // Initial load when assistant changes; clear deleted tracking on switch
  useEffect(() => {
  ...
  const handleDelete = async (fileId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/documents?fileId=${fileId}&assistantId=${selectedId}`,
        { method: 'DELETE' }
      );

      if (res.ok) {
        // Register as deleted BEFORE updating state so polling can't restore it
        deletedIds.current.add(fileId);
  ...
  if (!loading && assistants.length === 0) {
    return (
      <div className="text-center py-16 text-gray-500">
        ...
        <p className="text-sm mt-1">Create an assistant first to manage documents.</p>
      </div>
    );
  }
      ...
      >
        ...
        {uploading ? (
          ...
          <>
            ...
            <p className="text-xs text-gray-400 mt-1">PDF, TXT, MD, DOCX, CSV supported</p>
          </>
        )}
      </div>
