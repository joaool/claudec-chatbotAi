...
interface Conversation {
  _id: string; sessionId: string; question: string; answer: string;
  assistantName: string; sources: string[]; userIp: string;
  country: string; regionName: string; city: string; timestamp: string;
}

export default function ClientAnalyticsTable({ slug }: { slug: string }) {
  ...
  const fetchData = useCallback(async (f: typeof applied, p: number) => {
    ...
    try {
      ...
      if (res.ok) { const d = await res.json(); setConversations(d.conversations); setTotalPages(d.totalPages); setTotal(d.total); }
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && conversations.length === 0 ? <p className="text-sm text-gray-400 text-center py-12">No conversations found.</p> : (
